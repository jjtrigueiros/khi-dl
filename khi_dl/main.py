...
class KhiTrack:
    ...
    async def get_source(self, session: aiohttp.ClientSession) -> URL:
        ...
        return URL(BeautifulSoup(khi_track_page, features="html.parser").find(id="audio").get("src"))
    
    async def get_audio_stream(self, session: aiohttp.ClientSession) -> t.AsyncIterator[bytes]:
        audio_src = await self.get_source(session=session)
        async with session.get(audio_src) as response:
            async for chunk in response.content.iter_chunked(1024):
                yield chunk

    async def save_to_file(self, session: aiohttp.ClientSession, filename: Path) -> None:
    ...
    def generate_filename(self)-> str:
        ...
        identifier = f"{self.disc}-{self.number:02d}" if self.disc else f"{self.number:02d}"
...
async def main(album_urls: list[URL], dl_dir: Path):
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        async with asyncio.TaskGroup() as tg:
            tasks = [tg.create_task(KhiAlbum.from_khi_url(khi_url=url, session=session)) for url in album_urls]
            ...
            for album in albums:
                ...
                for track in album.tracks:
                    ...
                    if track_file.exists():
                        ...
                        continue
                    tg.create_task(track.save_to_file(session=session, filename=track_file))
